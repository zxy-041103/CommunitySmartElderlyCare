<template>
  <div class="page-card">
    ...
    <el-table :data="tableData" border stripe>
      ...
      <el-table-column prop="status" label="状态" width="90"><template #default="{row}"><el-tag :type="{UPCOMING:'primary',ONGOING:'success',ENDED:'info'}[row.status]">{{ {UPCOMING:'即将开始',ONGOING:'进行中',ENDED:'已结束',CANCELLED:'已取消'}[row.status] }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id?'编辑活动':'新增活动'" width="600px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="状态"><el-select v-model="form.status" style="width:100%"><el-option label="即将开始" value="UPCOMING" /><el-option label="进行中" value="ONGOING" /><el-option label="已结束" value="ENDED" /></el-select></el-form-item>
