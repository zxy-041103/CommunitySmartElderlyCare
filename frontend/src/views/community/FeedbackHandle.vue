<template>
  <div class="page-card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      <h3 class="page-title" style="margin-bottom:0">反馈处理</h3>
    ...
    <el-table :data="tableData" border stripe>
      ...
      <el-table-column label="操作" width="120" fixed="right">
        <template #default="{row}"><el-button size="small" type="primary" @click="openReply(row)" v-if="row.status==='PENDING'">回复</el-button></template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="replyVisible" title="回复反馈" width="450px">
      ...
      <template #footer><el-button @click="replyVisible=false">取消</el-button><el-button type="primary" @click="submitReply">确定</el-button></template>
    </el-dialog>
  </div>
</template>
...
import { feedbackPage, feedbackReply } from '../../api'
import { ElMessage } from 'element-plus'
const query = ref({ pageNum: 1, pageSize: 10, status: '' }); const tableData = ref([]); const total = ref(0); const replyVisible = ref(false); const replyForm = ref({})
...
const openReply = (row) => { replyForm.value = { id: row.id, content: row.content, reply: '' }; replyVisible.value = true }
const submitReply = async () => { await feedbackReply(replyForm.value.id, replyForm.value); ElMessage.success('回复成功'); replyVisible.value = false; loadData() }
onMounted(loadData)
</script>
