...
public interface ElderlyEmergencyMapper extends BaseMapper<EmergencyHelp> {
    ...
    @Select("SELECT eh.* FROM emergency_help eh LEFT JOIN sys_user u ON eh.user_id = u.id WHERE (u.username LIKE CONCAT('%', #{name}, '%') OR #{name} IS NULL) AND (eh.help_type = #{type} OR #{type} IS NULL) AND (eh.help_status = #{status} OR #{status} IS NULL) ORDER BY eh.create_time DESC LIMIT #{offset}, #{size}")
